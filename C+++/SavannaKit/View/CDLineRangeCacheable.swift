//
//  CDLineRangeCacheable.swift
//  C+++
//
//  Created by 23786 on 2021/3/27.
//  Copyright © 2021 Zhu Yixuan. All rights reserved.
//

import Foundation

protocol CDLineRangeCacheable: AnyObject {
    
    var string: NSString { get }
    var lineRangeCache: CDLineRangeCache { get set }
...
extension CDLineRangeCacheable {
    ...
    private func ensureLineRanges(upTo endIndex: Int) {
        ...
        while index <= min(upperPasreBound, string.length - 1) {
            ...
            guard index != string.length || String(string)[index - 1].isNewline else { break }
...
private extension CDLineRangeCache {
